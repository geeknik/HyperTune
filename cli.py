...
def analyze_results(results):
    """Analyze results to extract common terms and unique concepts."""
...
def explain_score(result):
    """Generate a human-readable explanation of a response's score breakdown."""
    explanation = f"Total Score: {result['total_score']:.2f}\n"
    explanation += (
        f"  Coherence: {result['coherence_score']:.2f} (40% weight) - "
        "Measures how well the sentences flow and connect.\n"
    )
    explanation += (
        f"  Relevance: {result['relevance_score']:.2f} (40% weight) - "
        "Measures how closely the response relates to the prompt.\n"
    )
    explanation += (
        f"  Complexity: {result['complexity_score']:.2f} (20% weight) - "
...
def plot_score_comparison(results):
    """Create a bar chart comparing scores of top 3 responses."""
    labels = ["Coherence", "Relevance", "Complexity"]
    data = np.array(
        [
            [r["coherence_score"], r["relevance_score"], r["complexity_score"]]
            for r in results[:3]
        ]
    )
...
def plot_hyperparameter_impact(results):
    """Create scatter plots showing hyperparameter impact on total score."""
    df = pd.DataFrame(
        [
            {
...
def main():
    """Main entry point for the HyperTune CLI."""
    ...
    parser.add_argument(
        "--model", help="Specific model to use (optional, uses provider default)"
    )
    parser.add_argument(
        "--list-providers",
        action="store_true",
    ...
    for param in ["temperature", "top_p", "frequency_penalty", "presence_penalty"]:
        ...
        print(
            f"  {param}: {direction} correlation ({correlation:.2f}) with total score"
        )

    print("\nKey Takeaways:")
    print(
        "1. The top response provides the most balanced explanation, "
        "scoring well across all metrics."
    )
    top_terms = ", ".join([term for term, _ in common_terms[:5]])
    print(f"2. Common themes across responses include: {top_terms}")
    print(
        "3. The complexity of explanations varies, with some responses "
        "using more technical language than others."
    )
    print(
        "4. All top responses maintain high relevance to the prompt, "
        "ensuring focused explanations."
    )
    print(
        f"5. The best performing set of hyperparameters achieved a total "
        f"score of {best_result['total_score']:.2f}"
    )

    print("\nRecommendations for Further Tuning:")
    print(
        "1. Experiment with narrower ranges of hyperparameters around "
        "the best performing values."
    )
    print(
        "2. Consider increasing the number of iterations to explore more "
        "hyperparameter combinations."
    )
    print(
        "3. Analyze the trade-offs between different scoring components "
        "(coherence, relevance, complexity) and adjust weights if needed."
    )
    ...
    print(
        "Hyperparameters are randomly selected for each iteration to explore "
